...
from gazjango.issues.views         import show_combined
...
class Command(SendingOutCommand):
    ...
    def set_content(self, dummy_request):
        try:
            html_response = show_combined(dummy_request)
        ...
        self.text_content = show_combined(dummy_request, plain=True).content
    
    def get_subject(self):
        now = datetime.datetime.now()
        return now.strftime("%%A, %%B %d" % now.day)
    
    def get_sent_str(self):
        now = datetime.datetime.now()
        return now.strftime("%Y-%m-%d-" + ('1' if now.hour < 13 else '2'))
