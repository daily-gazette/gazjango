...
from gazjango.issues.views         import show_events
...
class Command(SendingOutCommand):
    ...
    def set_content(self, dummy_request):
        try:
            html_response = show_events(dummy_request)
        ...
        self.text_content = show_events(dummy_request, plain=True).content
    
    def get_subject(self):
        now = datetime.datetime.now()
        return now.strftime("%%A, %%B %d - Events" % now.day)
