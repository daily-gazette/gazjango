...
class RegistrationWithProfileBackend(DefaultBackend):
    def register(self, request, **kwargs):
        """
        Creates a User and UserProfile for a new registration, based on `kwargs`.
        
        All of the User logic from the superclass is done; then we set the User's
        `first_name` and `last_name` (based on a single `name` argument), and
        create a UserProfile based on `kind` (one of UserKind.KINDS) and, optionally,
        `year`. We also set the User's groups.
        """
        user = super(RegistrationWithProfileBackend, self).register(request, **kwargs)
